...
void turnToAngle(double turn_angle, double time_limit_msec, bool exit, double max_output) {
    ...
    // Reset the inertial sensor heading
    inertial_sensor.resetRotation();
    wait(50, msec); // give it a short moment to reset

    double threshold = 1;  // acceptable error in degrees
    PID pid = PID(turn_kp, turn_ki, turn_kd);

    // Keep negative angles; normalize to -180 -> 180
    ...
    // Visualization setup
    double draw_amplifier = 230 / fmax(fabs(turn_angle), 1); // avoid division by zero
    ...
    double output;
    double current_heading, previous_heading = getInertialHeading();
    int index = 1;
    ...
    while(Brain.timer(msec) - start_time <= time_limit_msec) {
        ...
        double error = turn_angle - current_heading;
        ...
        // Visualization
        Brain.Screen.drawLine(index * 3, fabs(previous_heading) * draw_amplifier,
                              (index + 1) * 3, fabs(current_heading) * draw_amplifier);
        previous_heading = current_heading;
        index++;

        // Check if target reached
        if(pid.targetArrived()) break;

        wait(10, msec);
    }

    if(exit) stopChassis(vex::hold);
...
void moveToPoint(double x, double y, int dir, double time_limit_msec, bool exit, double max_output, bool overturn) {
    ...
    // Reset the inertial sensor heading
    inertial_sensor.resetRotation();
    wait(50, msec); // small delay to allow reset to take effect
    ...
    while (Brain.timer(msec) - start_time <= time_limit_msec) {
        ...
        if(overturn_value > 0 && overturn) {
            if(left_output > 0) left_output -= overturn_value;
            else left_output += overturn_value;
        }
